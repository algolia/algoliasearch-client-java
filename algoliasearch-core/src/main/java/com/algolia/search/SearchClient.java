...
public final class SearchClient
   ...
   * @throws AlgoliaRuntimeException When an error occurred during the serialization
   */
  public void waitTask(long taskID, RequestOptions requestOptions) {
    waitTask(taskID, 100, requestOptions);
  }

  /**
   * Wait for a task to complete before executing the next line of code. Currently supports
   * dictionary tasks only. All write operations in Algolia are asynchronous by design.
   *
   * @param taskID The Algolia taskID
   * @param timeToWait The time to wait between each call
   * @param requestOptions Options to pass to this request
  ...
  /**
   * Get the status of the given task. Currently supports * dictionary tasks only.
   *
   * @param taskID The Algolia taskID
   * @param requestOptions Options to pass to this request
   * @throws AlgoliaRetryException When the retry has failed on all hosts
